...
**Governance and project health.** BIDS has formal, documented governance: an elected **Steering Committee** (PI, co-PIs Poline, Poldrack, Pestilli, and Ray are current or Emeritus members), a **Maintainers** group with defined roles and merge authority, a public **BEP process** (`bids-standard/bids-extensions`) with documented review and merge gates. PI leads published roadmap (`bids-standard/bids-2-devel` and BIDS 2.0 Project). Every named project in this proposal has active CI, public issue triage, Maintainers-level review on every merge, and a public release cadence — collectively the spec + apps + tooling repositories have 300+ contributors and a decade of sustained activity.

**Funder lineage.** BIDS itself was not a CZI EOSS grantee, but multiple BIDS-adjacent open-source projects that directly depend on or consume BIDS were EOSS-funded across the program's lifespan: **MNE-Python** (Cycle 2), **Nilearn** (Cycle 5, PI Poline; awarded Nov 2022, 2-yr term), **NiPreps/fMRIPrep** (Cycle 5, PI Esteban; awarded Nov 2022, 2-yr term), and **NeuroDesk** (Cycle 6, awarded 2024, 2-yr term). **Two of those EOSS PIs (Poline, Esteban) are co-PIs on this proposal** — direct continuity from EOSS into the OS4LS successor program. In 2023, the **BIDS Steering Committee**, representing BIDS project, received **The Neuro – Irv and Helga Cooper Foundation Open Science International Prize** (citation: *"for its longstanding community-driven approach to standardizing neuroimaging data, resulting in widespread adoption"*); the PI is one of five named recipients (with Rokem/U.Washington, Pernet/Copenhagen, Niso/Cajal Institute, Oostenveld/Donders).
